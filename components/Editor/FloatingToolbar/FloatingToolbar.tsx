...
export const FloatingToolbar: React.FC<FloatingToolbarProps> = ({
  ...
  const calculatePosition = useCallback((): Position | null => {
    ...
    // 检查是否选中的是块级元素（图片、折叠块等）
    const { state } = editor;
    const { $from, $head } = state.selection;
    const node = $from.parent;
    
    // 如果选中的是 customImage、customDetails、youtube、mermaid 等块级元素，不显示工具栏
    // 注意：表格中选中文字时，应该显示文本工具栏，所以不检查 table
    if (editor.isActive('customImage') || 
        editor.isActive('customDetails') || 
        editor.isActive('youtube') ||
        editor.isActive('mermaid')) {
      return null;
    }
    
    // 如果在表格中，但选中的是单元格而不是文字，不显示文本工具栏
    if (editor.isActive('table')) {
      const isCellSelection = state.selection.constructor.name.includes('CellSelection');
      if (isCellSelection) {
        return null;
      }
    }
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="floating-toolbar dark:bg-gray-800"
        >
