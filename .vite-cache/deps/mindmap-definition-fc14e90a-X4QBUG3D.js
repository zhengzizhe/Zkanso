...
var addNode = (level, id, descr, type) => {
  var _a, _b;
  log$1.info("addNode", level, id, descr, type);
  const conf = getConfig();
  let padding = ((_a = conf.mindmap) == null ? void 0 : _a.padding) ?? defaultConfig$2.mindmap.padding;
  switch (type) {
  ...
  const node = {
    id: cnt++,
    nodeId: sanitizeText$2(id, conf),
    level,
    descr: sanitizeText$2(descr, conf),
    type,
    children: [],
    width: ((_b = conf.mindmap) == null ? void 0 : _b.maxNodeWidth) ?? defaultConfig$2.mindmap.maxNodeWidth,
    padding
  };
...
var decorateNode = (decoration) => {
  ...
  if (decoration.icon) {
    node.icon = sanitizeText$2(decoration.icon, config);
  }
  if (decoration.class) {
    node.class = sanitizeText$2(decoration.class, config);
  }
};
var type2Str = (type) => {
  switch (type) {
...
var db$1 = db;
var MAX_SECTIONS = 12;
var defaultBkg = function(db2, elem, node, section) {
  const rd = 5;
...
var rectBkg = function(db2, elem, node) {
...
var cloudBkg = function(db2, elem, node) {
  const w = node.width;
  const h = node.height;
  const r1 = 0.15 * w;
  const r2 = 0.25 * w;
...
var bangBkg = function(db2, elem, node) {
  const w = node.width;
  const h = node.height;
  const r = 0.15 * w;
  elem.append("path").attr("id", "node-" + node.id).attr("class", "node-bkg node-" + db2.type2Str(node.type)).attr(
...
var circleBkg = function(db2, elem, node) {
...
var hexagonBkg = function(_db, elem, node) {
  const h = node.height;
  const f = 4;
  const m = h / f;
...
var roundedRectBkg = function(db2, elem, node) {
...
var drawNode = function(db2, elem, node, fullSection, conf) {
...
var positionNode = function(db2, node) {
...
function positionNodes(db2, cy) {
  cy.nodes().map((node, id) => {
    ...
    log$1.info("Id:", id, "Position: (", node.position().x, ", ", node.position().y, ")", data);
    el.attr(
      "transform",
...
var draw = async (text, id, _version, diagObj) => {
  var _a, _b;
  ...
  setupGraphViewbox$1(
    void 0,
    svg,
    ((_a = conf.mindmap) == null ? void 0 : _a.padding) ?? defaultConfig$2.mindmap.padding,
    ((_b = conf.mindmap) == null ? void 0 : _b.useMaxWidth) ?? defaultConfig$2.mindmap.useMaxWidth
  );
};
var renderer = {
  draw
...
//# sourceMappingURL=mindmap-definition-fc14e90a-X4QBUG3D.js.map
